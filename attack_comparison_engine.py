...
class AttackComparisonEngine:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                if tier == ProtectionTier.UNPROTECTED:
                    ...
                    if response.status_code == 200:
                        ...
                        return {"available": True, "status": health_data.get("status", "unknown"), "details": health_data}
                    else:
    ...
    async def run_comprehensive_comparison(self, attack_types: Optional[List[str]] = None) -> Dict[str, Dict[ProtectionTier, AttackResult]]:
        ...
        for attack_type in attack_types:
            try:
                ...
                logger.info(f"  ☁️ Cloud AI: {'BLOCKED' if cloud.blocked else 'ALLOWED'} (Score: {cloud.threat_score:.2f})")
                logger.info(f"  🏠 Local AI: {'BLOCKED' if local.blocked else 'ALLOWED'} (Score: {local.threat_score:.2f})")
    ...
    def generate_comparison_report(self) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                "total_attacks_tested": sum(m["total"] for m in self.metrics.values()),
                "attacks_by_tier": {}
            },
            "metrics": {},
            "effectiveness": {},
