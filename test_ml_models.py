...
def test_feature_extraction():
    ...
    print(
        f"   Sample features: sql_risk_score={features.get('sql_risk_score', 0):.2f}, "
    ...
    print(
        f"   Risk scores: sql={mal_features.get('sql_risk_score', 0):.2f}, "
...
def test_anomaly_detection():
    ...
    print(f"   Normal request: anomaly={normal_result['is_anomaly']}, " f"score={normal_result['anomaly_score']:.2f}")
    ...
    print(
        f"   Anomalous request: anomaly={anomaly_result['is_anomaly']}, " f"score={anomaly_result['anomaly_score']:.2f}"
    )
...
def test_attack_classification():
    ...
    # Add normal requests
    for i in range(20):
        training_data.append({"method": "GET", "path": f"/api/resource/{i}", "query": "", "body": ""})
        labels.append("normal")

    # Add SQL injection attempts
    for i in range(20):
        training_data.append(
            {"method": "POST", "path": "/api/login", "query": f"id={i} OR 1=1", "body": "username=admin' OR '1'='1"}
        )
    ...
    for i in range(20):
        training_data.append(
            {"method": "POST", "path": "/api/comment", "query": "", "body": f'<script>alert("XSS{i}")</script>'}
        )
        labels.append("xss")
    ...
    # Test classification
    test_cases = [
        {"request": {"method": "GET", "path": "/api/users", "query": "", "body": ""}, "expected": "normal"},
        {
            "request": {"method": "POST", "path": "/api/query", "query": "search='; DROP TABLE users; --", "body": ""},
    ...
    for test in test_cases:
        ...
        print(
            f"   Test {test['expected']}: detected={result['attack_type']}, " f"confidence={result['confidence']:.2f}"
        )
