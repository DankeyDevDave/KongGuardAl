...
class SupabaseFinal:
    ...
    def execute_sql_file(self, sql_content: str) -> dict[str, Any]:
        ...
        try:
            ...
            copy_result = subprocess.run(copy_cmd, shell=True, capture_output=True, text=True, check=False)
            ...
            exec_result = subprocess.run(exec_cmd, shell=True, capture_output=True, text=True, timeout=30, check=False)

            # Cleanup
            os.unlink(temp_file)
            cleanup_cmd = f"ssh root@{self.host} 'rm -f {remote_file}'"
            subprocess.run(cleanup_cmd, shell=True, capture_output=True, text=True, check=False)

            return {"success": exec_result.returncode == 0, "output": exec_result.stdout, "error": exec_result.stderr}
