...
class HackathonDemoRecorder:
    ...
    async def execute_action(self, action: Dict[str, Any], scene: Dict[str, Any]):
        ...
        if action_type == 'wait':
            ...
            await self.click_with_visual(
                action['selector'],
                description,
                action.get('wait_after', 2)
            )
...
Examples:
  ...
  python hackathon_demo_recorder.py --scenes 1,3,5
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=True,
        help='Capture screenshots at each stage'
    )
    parser.add_argument(
        '--no-screenshots',
        action='store_true',
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=True,
        help='Use narrator timing from config'
    )
    parser.add_argument(
        '--config',
        default='narrator_timing.json',
        help='Path to timing configuration file'
    )
    parser.add_argument(
        '--scenes',
        help='Comma-separated scene numbers to record (e.g., "1,3,5")'
    )
    ...
    if args.scenes:
        ...
        recorder.config['scenes'] = [
            s for s in recorder.config['scenes'] 
            if s['number'] in scene_numbers
        ]
