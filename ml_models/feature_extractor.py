...
class FeatureExtractor:
    ...
    def __init__(self):
        ...
        self.sql_pattern = re.compile(
            r"(select|union|drop|insert|update|delete|exec|declare|cast|convert|waitfor|delay|benchmark)", re.IGNORECASE
        )
        self.xss_pattern = re.compile(
            r"(<script|javascript:|onerror=|onclick=|onload=|<iframe|alert\(|prompt\(|confirm\()", re.IGNORECASE
        )
    ...
    def extract_behavioral_features(self, request_data: dict[str, Any]) -> dict[str, float]:
        ...
        features["is_browser"] = (
            1.0 if any(browser in user_agent.lower() for browser in ["chrome", "firefox", "safari", "edge"]) else 0.0
        )
        features["is_mobile"] = (
            1.0 if any(mobile in user_agent.lower() for mobile in ["mobile", "android", "iphone"]) else 0.0
        )
    ...
    def extract_statistical_features(self, request_data: dict[str, Any]) -> dict[str, float]:
        ...
        features["request_complexity"] = (
            features["body_to_path_ratio"] * 0.3 + header_count * 0.1 + request_data.get("query_param_count", 0) * 0.2
        )
    ...
    def create_feature_vector(self, features: dict[str, float], feature_names: list[str]) -> np.ndarray:
        """
