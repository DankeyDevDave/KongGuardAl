...
class ModelManager:
    ...
    def __init__(self, model_dir: str = "models/trained"):
        ...
        self.model_versions = {"anomaly": "1.0.0", "classifier": "1.0.0", "features": "1.0.0"}
    ...
    def analyze_request(self, request_data: dict[str, Any]) -> dict[str, Any]:
        ...
        threat_score = self._calculate_threat_score(features, anomaly_result, attack_result)
        ...
        # Cache result
        self.prediction_cache[cache_key] = {"timestamp": datetime.now(), "result": analysis}

        # Store for training
        if self.auto_train_enabled:
            self.training_buffer.append(
                {"request": request_data, "features": features, "analysis": analysis, "timestamp": datetime.now()}
            )
    ...
    def _calculate_threat_score(
        self, features: dict[str, float], anomaly_result: dict[str, Any], attack_result: dict[str, Any]
    ) -> float:
        """
        ...
        feature_score = 0
        risk_features = ["sql_risk_score", "xss_risk_score", "path_risk_score", "cmd_risk_score", "overall_risk_score"]
    ...
    def _determine_action(self, threat_score: float, attack_result: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        if threat_score >= 0.9:
            ...
            return {"action": "allow", "reason": "Normal traffic", "confidence": "high", "additional": []}
    ...
    def train_models(self, training_data: list[dict[str, Any]], labels: Optional[dict[str, list]] = None):
        """
        Train all ML models
    ...
    def save_models(self):
        ...
        try:
            ...
            self.anomaly_detector.save_model(self.model_dir / "anomaly_detector.joblib")

            # Save attack classifier
            self.attack_classifier.save_model(self.model_dir / "attack_classifier.joblib")
    ...
    def get_model_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                ...
                "avg_processing_time_ms": np.mean(self.metrics["processing_times"])
    ...
    def update_threat_intelligence(self, threat_data: dict[str, Any]):
        ...
        logger.info(
            f"Threat intelligence updated: {len(self.threat_patterns)} patterns, "
            f"{len(self.known_attackers)} known attackers"
        )

    def provide_feedback(self, request_id: str, correct_label: str, was_correct: bool):
        """
        ...
        logger.info(f"Feedback recorded. Current accuracy: " f"{np.mean(self.metrics['accuracy_history']):.2%}")
