...
class AttackClassifier:
    ...
    def train(self, training_data: list[dict[str, Any]], labels: list[str]):
        ...
        if hasattr(self.model, "feature_importances_"):
            self.feature_importance = {
                f"feature_{i}": importance for i, importance in enumerate(self.model.feature_importances_)
            }
    ...
    def predict(self, request_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not self.is_trained:
            return {"attack_type": "unknown", "confidence": 0.0, "probabilities": {}, "reason": "Model not trained"}
        ...
        prob_dist = {self.label_encoder.inverse_transform([i])[0]: float(prob) for i, prob in enumerate(probabilities)}
    ...
    def _explain_classification(
        self, attack_type: str, probabilities: dict[str, float], request_data: dict[str, Any]
    ) -> str:
        """
