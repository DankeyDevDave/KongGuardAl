...
class AnomalyDetector:
    ...
    def extract_features(self, request_data: dict[str, Any]) -> dict[str, float]:
        ...
        features["has_sql_keywords"] = (
            1 if any(kw in body_str.lower() for kw in ["select", "drop", "union", "insert"]) else 0
        )
    ...
    def train(self, training_data: list[dict[str, Any]], labels: Optional[list[int]] = None):
        ...
        self.model = IsolationForest(
            contamination=self.contamination, n_estimators=self.n_estimators, random_state=42, n_jobs=-1
        )
    ...
    def update_model(self, feedback: dict[str, Any]):
        ...
        self.training_data.append({"request": request_data, "label": label, "timestamp": datetime.now().isoformat()})
