...
class SQLiteAdapter(DatabaseAdapter):
    ...
    def save_attack_metric(self, metric: AttackMetric) -> None:
        ...
        ''', (
            metric.run_id, metric.tier, metric.attack_type, metric.attack_category,
            metric.payload, metric.response_time_ms, metric.threat_score,
            metric.confidence, metric.action_taken, metric.blocked,
            metric.status_code, metric.source_ip, metric.user_agent, metric.error_message
        ))
    ...
    def save_attack_metrics_batch(self, metrics: List[AttackMetric]) -> None:
        ...
        data = [
            (m.run_id, m.tier, m.attack_type, m.attack_category, m.payload,
             m.response_time_ms, m.threat_score, m.confidence, m.action_taken,
             m.blocked, m.status_code, m.source_ip, m.user_agent, m.error_message)
            for m in metrics
        ]
    ...
    def complete_attack_run(self, run_id: int, total_attacks: int) -> None:
        ...
        for stats in tier_stats:
            ...
            ''', (run_id, tier, total, blocked, allowed, avg_time, 
                  detection_rate, min_score, max_score, avg_score))
