    ...
    def __init__(self):
        ...
        self.threat_patterns = {
            "sql_injection": [
                ...
                "alter table", "exec(", "sp_", "xp_", "information_schema"
            ],
            "xss": [
            ...
            "command_injection": [
                ...
                "nc ", "netcat", "wget ", "curl ", "/bin/sh", "/bin/bash",
                "ping ", "nslookup", "dig ", "whoami", "id", "ps aux"
            ],
            "path_traversal": [
                "../", "..\\", "%2e%2e%2f", "%2e%2e%5c", "....//",
                "/etc/passwd", "/etc/shadow", "\\windows\\system32",
                "boot.ini", "web.config", ".htaccess", "php.ini"
            ],
            "ldap_injection": [
                "*)(&", "*)(|", "admin*", "*)(cn=*", "*)(objectclass=*)",
                "*(|(objectclass=*))", "*))%00", "*))(|", "*)(uid=*"
            ]
        }
...
async def analyze_threat(request: ThreatAnalysisRequest):
    ...
    try:
        ...
        logger.info(f"Local analysis completed: {analysis['threat_type']} (score: {threat_score:.2f}, time: {processing_time:.3f}s)")
