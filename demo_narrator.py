...
class DemoNarrator:
    ...
    async def get_presentation_script(self) -> str:
        ...
        for i, segment in enumerate(self.presentation_segments, 1):
            ...
            if segment.presenter_notes:
                script_lines.extend([
                    f"**Presenter Notes**: {segment.presenter_notes}",
                    ""
                ])
    ...
    async def export_presentation_materials(self, output_dir: str = "presentation_materials"):
        ...
        notes_lines = [
            f"# Kong Guard AI - {self.mode.value.title()} Speaker Notes",
            "=" * 50,
            ""
        ]
...
async def main():
    ...
    parser.add_argument("--mode", choices=["executive", "technical", "industry"], 
                       default="executive", help="Presentation mode")
    parser.add_argument("--export", action="store_true", 
                       help="Export presentation materials")
    parser.add_argument("--run", action="store_true",
                       help="Run live presentation")
