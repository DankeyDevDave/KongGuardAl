...
class ConnectionManager:
    ...
    async def broadcast_threat_analysis(self, analysis_data: dict):
        ...
        logger.info(
            f"Broadcasting to {len(self.active_connections)} WebSocket clients: {analysis_data.get('threat_type', 'unknown')}"
        )
        ...
        await self.broadcast(
            {"type": "threat_analysis", "data": analysis_data, "metrics": self.metrics, "event": threat_event}
        )

    async def send_ai_thinking(self, thinking_data: dict):
        """Send AI thinking process updates"""
        await self.broadcast({"type": "ai_thinking", "data": thinking_data})
...
class GeminiProvider(AIProvider):
    ...
    async def analyze(self, features: RequestFeatures, context: RequestContext) -> dict:
        ...
        await manager.send_ai_thinking({"status": "starting", "message": "AI analysis initiated", "model": self.model})
    ...
    async def _intelligent_fallback(self, features: RequestFeatures, context: RequestContext) -> dict:
        ...
        await manager.send_ai_thinking({"status": "fallback", "message": "Using intelligent pattern analysis"})
...
async def simulate_attacks():
    ...
    for attack in attacks:
        request = ThreatAnalysisRequest(features=attack["features"], context=RequestContext())

        # Simulate delay for visual effect
        await asyncio.sleep(1)

        result = await analyze_threat(request)
        results.append({"attack": attack["name"], "result": result.dict()})
...
if __name__ == "__main__":
    ...
    print("🔍 API: http://localhost:18002")
    print("🖥️  Dashboard: http://localhost:18002/dashboard")
    print("🔌 WebSocket: ws://localhost:18002/ws")
    print("")
