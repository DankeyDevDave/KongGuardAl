...
class VideoPresentationAutomation:
    ...
    async def initialize_browser(self, headless=False, record_video=True):
        ...
        if record_video:
            ...
            browser_options["record_video"] = {"dir": str(self.recording_path), "size": {"width": 1920, "height": 1080}}
    ...
    async def run_comprehensive_demo(self):
        ...
        await self.show_narration("Welcome to Kong Guard AI - Enterprise Three-Tier Protection Demonstration", 6)
        ...
        await self.show_narration("We'll test multiple attack types to demonstrate AI-powered threat detection", 5)

        # Execute each attack scenario
        for i, scenario in enumerate(self.attack_scenarios, 1):
            await self.show_narration(f"Attack Scenario {i} of {len(self.attack_scenarios)}", 3)

            await self.execute_attack_comparison(scenario)

            # Show financial impact
            await self.show_narration(f"Financial Impact Prevented: {scenario['financial_impact']}", 4)
        ...
        await self.show_narration("Ready to protect your APIs with Kong Guard AI? Contact us for implementation!", 6)
...
async def main():
    ...
    parser.add_argument(
        "--dashboard-url",
        default="http://localhost:8090/enterprise_demo_dashboard.html",
